...
px = 1705;  py = 330;  pw = 1000;  ph = 400;  % window location and dimensions
ph2 = floor(ph/2);  s = 8;
...
rad = 47;                               % radius of circular target
for i = 1:pw
    for j = 1:ph
...
if PR N=3; end                      % If in calibration mode
LN = 5;                             % number of lightness levels per repeat

Lmat=zeros(N,LN);
for i= 1:N
    Lmat(i,:) = randperm(LN)*10+20; % matrix of randomly permuted lightness levels
                                    % gives 30:10:70
end
        ...
        while ~BUTTON_PRESSED
            ...
            if butval == ON
                ...
                if reset_colour == 1
                    ...
                    fname2 = fullfile(cd,sprintf('Spectrum%03d.mat',PRn));
                    save(fname2,'PRspc')
                    end
                end
...
filename = fullfile('C:','Test','Experiment - Jul 2017',[fname,'.txt']);
fp = fopen(filename,'w');
...
Wref = [Xw Yw Zw];
filename2 = fullfile('C:','Test','Experiment - Jul 2017',fname);
save(filename2,'Tmatch','LABmatch','RGBmatch','RGBstart','Wref');  % save file
